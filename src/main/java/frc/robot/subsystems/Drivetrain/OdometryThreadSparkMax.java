 ...
 * to a set of queues.
 */
public class OdometryThreadSparkMax {
    ...
    private static OdometryThreadSparkMax instance = null;
    private Notifier notifier = new Notifier(this::run);

    public static OdometryThreadSparkMax getInstance() {
        if (instance == null) {
            instance = new OdometryThreadSparkMax();
        }
        return instance;
    }

    private OdometryThreadSparkMax() {
        notifier.setName("OdometryThread");
    }

    public void start() {
        if (timestampQueues.size() > 0) {
            notifier.startPeriodic(1.0 / SwerveDriveConstants.DriveConstants.kOdometryFrequency);
        }
    }
